...
SPAN_FACTOR = 2
LIST_W_CATEG_MAPPING = OrderedDict(
    (
        ("id", Feed.id),
        ("str", Feed.title),
...
class FeedController(AbstractController):
    ...
    def _set_unread_count(self, filters: set, fctr: int):
        q = (
            session.query(Article.feed_id, func.count(Article.id))
            .join(
                Cluster,
                and_(
    ...
    def list_w_categ(self):
        ...
        for row in (
            session.query(*LIST_W_CATEG_MAPPING.values())
            .filter(
    ...
    def list_late(self, limit=0):
        ...
        filters = self._to_filters(
            ...
            __or__=[
                {"expires__lt": now, "expires__ne": None},
                {
        ...
        if feeds:
            for feed in feeds:
                if feed.last_retrieved == UNIX_START:
                    continue
                FEED_LATENESS.labels(feed_type=feed.feed_type.value).observe(
                    (now - feed.last_retrieved).total_seconds()
                )
            self.update(
                {"id__in": [feed.id for feed in feeds]},
                {"last_retrieved": now},
            )
        return feeds
    ...
    def __clean_feed_fields(self, attrs):
        ...
        if "filters" in attrs:
            attrs["filters"] = [
                filter_
                for filter_ in (attrs["filters"] or [])
    ...
    def __update_default_expires(self, feed, attrs):
        ...
        try:
            ...
            if not attrs["expires"].tzinfo:
                ...
                attrs["expires"] = max_expires
                logger.debug(
                    "%r expiring too late, forcing expire in %ds",
                    feed,
                    conf.feed.max_expires,
                )
            elif attrs["expires"] < min_expires:
                method = "from header min limited"
                attrs["expires"] = min_expires
                logger.debug(
                    "%r expiring too early, forcing expire in %ds",
                    feed,
                    conf.feed.min_expires,
                )
        except Exception:
            attrs["expires"] = max_expires
            method = "defaulted to max"
    ...
    def delete(self, obj_id, commit=True):
        ...
        logger.info(
            "DELETE %r - removing back ref from cluster to article", feed
        )
        art_ids = self.__actrl.read(feed_id=obj_id).with_entities(
            self.__actrl._db_cls.id
        )
        clu_ctrl.update(
            {"user_id": feed.user_id, "main_article_id__in": art_ids},
        ...
        session.execute(
            delete(Article).where(
                Article.feed_id == feed.id, Article.user_id == feed.user_id
            )
        )
