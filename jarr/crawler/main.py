...
def feed_cleaner(feed_id):
    ...
    try:
        ...
        logger.exception(
            "something went wrong when deleting feeds %r", feed_id
        )
...
def metrics_users_active():
    ...
    threshold_connection = datetime.now(UTC) - timedelta(
        days=conf.feed.stop_fetch
    )
    active = UserController().read(
        is_active=True, last_connection__ge=threshold_connection
    )
...
def metrics_users_long_term():
    ...
    threshold_created = datetime.now(UTC) - timedelta(
        days=conf.feed.stop_fetch + 1
    )
    long_term = UserController().read(
        is_active=True,
...
def scheduler():
    ...
    for feed in feeds:
        logger.debug(
            "%r: scheduling to be fetched on queue:%r", feed, queue.value
        )
    ...
    for user_id in ArticleController.get_user_id_with_pending_articles():
        if REDIS_CONN.setnx(JARR_CLUSTERIZER_KEY % user_id, "true"):
            REDIS_CONN.expire(
                JARR_CLUSTERIZER_KEY % user_id, conf.crawler.clusterizer_delay
            )
            logger.debug(
                "Scheduling clusterizer for User(%d) on queue:%r",
                user_id,
                queue.value,
            )
