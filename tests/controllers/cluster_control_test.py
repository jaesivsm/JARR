...
class ClusterControllerTest(BaseJarrTest):
    ...
    def test_cluster_tfidf_control(self):
        conf.clustering.tfidf.min_vector_size = 2
        article = ArticleController().read(category_id__ne=None).first()
        cluster = article.cluster
        ...
        feed1 = FeedController(cluster.user_id).create(
            title="new feed",
            cluster_conf={"min_score": -1, "min_sample_size": 1},
        )
        update_on_all_objs(
        ...
        article = self.create_article_from(
            cluster,
            feed1,
            link=f"{cluster.main_article.link} do not match link",
        )
        self.assertInCluster(article, cluster, ClusterReason.tf_idf)
        article = self.create_article_from(
            cluster,
            feed2,
            link=f"{cluster.main_article.link} do not match link either",
        )
        self.assertNotInCluster(article, cluster)
    ...
    def test_cluster_enabled(self):
        ...
        feed = (
            FeedController(cluster.user_id)
            .read(
                category_id__ne=None,
                id__nin=[art.feed_id for art in cluster.articles],
            )
            .first()
        )
        ...
        update_on_all_objs(
            articles=cluster.articles, feeds=[feed], cluster_enabled=True
        )
        article = self.create_article_from(cluster, feed)
        self.assertInCluster(article, cluster)
        ...
        update_on_all_objs(
            articles=cluster.articles, feeds=[feed], cluster_enabled=None
        )
        UserController().update(
            {"id": cluster.user_id}, {"cluster_enabled": False}
        )
        article = self.create_article_from(cluster, feed)
        self.assertNotInCluster(article, cluster)
    ...
    def test_cluster_same_feed(self):
        ...
        CategoryController().update(
            {"id": cluster.main_article.category.id},
            {"cluster_same_feed": False},
        )
        article = self.create_article_from(cluster, cluster.main_article.feed)
        self.assertNotInCluster(article, cluster)
        ...
        UserController().update(
            {"id": cluster.user_id}, {"cluster_same_feed": False}
        )
        article = self.create_article_from(cluster, cluster.main_article.feed)
        self.assertNotInCluster(article, cluster)
        # reenabling user, will cluster
        UserController().update(
            {"id": cluster.user_id}, {"cluster_same_feed": True}
        )
        article = self.create_article_from(cluster, cluster.main_article.feed)
        self.assertInCluster(article, cluster)
    ...
    def test_cluster_same_category(self):
        ...
        FeedController().update(
            {"id": feed.id}, {"cluster_same_category": False}
        )
        article = self.create_article_from(cluster, feed)
        self.assertNotInCluster(article, cluster)
        # category's disabled, won't cluster
        FeedController().update(
            {"id": feed.id}, {"cluster_same_category": None}
        )
        CategoryController().update(
            {"id": cluster.main_article.category.id},
            {"cluster_same_category": False},
        ...
        UserController().update(
            {"id": cluster.user_id}, {"cluster_same_category": False}
        )
        article = self.create_article_from(cluster, feed)
        self.assertNotInCluster(article, cluster)
        # reenabling user, will cluster
        UserController().update(
            {"id": cluster.user_id}, {"cluster_same_category": True}
        )
        article = self.create_article_from(cluster, feed)
        self.assertInCluster(article, cluster)
