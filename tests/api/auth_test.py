...
class AuthTest(JarrFlaskCommon):
    ...
    def test_password_recovery(self, mock_emails_send):
        ...
        self.assertIn(f"/auth/recovery/{self.user.login}/{self.user.email}/",
                      mail_content)
        self.assertIn('\n\nRegards,', mail_content)
        token = mail_content.split('/')[-1].split('\n\n')[0]
        self.assertEqual(
                token, self.uctrl.get(id=self.user.id).renew_password_token)
        ...
        self.assertStatusCode(204, resp)
        self.assertNotEqual(
            old_password, self.uctrl.get(id=self.user.id).password)
        self.assertEqual(
